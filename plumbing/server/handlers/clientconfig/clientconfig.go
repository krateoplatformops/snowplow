...
import (
	...
	"github.com/krateoplatformops/snowplow/plumbing/server/handlers/logger"
	apierrors "k8s.io/apimachinery/pkg/api/errors"
	"k8s.io/client-go/rest"
)

func New(authnNS string, verbose bool) func(next http.Handler) http.Handler {
	mid := &handler{
		authnNS: authnNS,
		verbose: verbose,
	}
...
func (h *handler) Handle(next http.Handler) http.Handler {
	return http.HandlerFunc(func(wri http.ResponseWriter, req *http.Request) {
		...
		ep, err := endpoints.FromSecret(context.Background(), sarc, fmt.Sprintf("%s-clientconfig", sub), h.authnNS)
		if err != nil {
